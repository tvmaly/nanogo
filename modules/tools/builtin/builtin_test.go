...
func TestAskUser(t *testing.T) {
	...
	coord := builtin.NewAskUserCoordinator(bus, "sess-1")
	src := builtin.NewSource(bus, coord, nil)
	...
	select {
	case evt := <-sub:
		payload, ok := evt.Payload.(builtin.AskUserPayload)
		if !ok {
...
func TestSpawn(t *testing.T) {
	...
	src := builtin.NewSource(nil, nil, runner)
...
func TestSpawnRole(t *testing.T) {
	...
	src2 := builtin.NewSource(nil, nil, runner2)
...
func TestAskUserSetsSessionWaiting(t *testing.T) {
	...
	coord := builtin.NewAskUserCoordinatorWithSession(bus, sess)
	sub := bus.Subscribe(ctx, event.AskUser)
	...
	select {
	case evt := <-sub:
		payload := evt.Payload.(builtin.AskUserPayload)

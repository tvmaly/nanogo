...
type metaRecord struct {
	Status  coresession.Status `json:"status"`
...
func (s *impl) SetWaiting(turnID string) <-chan string {
	...
	s.status, s.turnID, s.waitCh = coresession.StatusWaiting, turnID, ch
	return ch
}
...
func (s *impl) Resume(turnID, answer string) {
	...
	if ok {
		s.status, s.waitCh, s.turnID = coresession.StatusActive, nil, ""
	}
	s.mu.Unlock()
	if ok && ch != nil {
		ch <- answer
...
func (s *impl) GetStatus() coresession.Status {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.status
}
...
func NewStore(dir string, clock Clock) coresession.Store {
	if clock == nil {
		clock = realClock{}
	}
...
func (s *store) Create(id string) (coresession.Session, error) {
	return &impl{id: id, path: s.msgPath(id), metaP: s.metaPath(id),
		status: coresession.StatusActive, clock: s.clock, savedAt: s.clock.Now()}, nil
}

func (s *store) Load(id string) (coresession.Session, error) {
	...
	m := metaRecord{Status: coresession.StatusActive, SavedAt: s.clock.Now()}
...
func (s *store) GC(ctx context.Context, ttl time.Duration) {
	...
	for _, e := range entries {
		...
		if m.Status == coresession.StatusWaiting && m.SavedAt.Before(cutoff) {
